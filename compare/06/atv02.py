...
#input:
#médias para cada caracteristica de cada classe ex: [-2, 6, 6] é o vetor de médias da classe 1
...
#priors - probabilidade de um padrão pertencer a cada classe, ou seja, qtd de padrões gerados para
# cada classe se [0.6, 0.4] serão 60% da primeira classe e 40% da segunda
#seed - configura o numer pseudoaleatório para que possamos comprara resultados
#angulo - angulação do plot

#onput:
#matriz de caracteristicas x padrões dos dados gerados
#classe para cada padrão gerado sendo que os indices dessa lista correspondem respectivamente a coluna
#da matriz de dados.
...
m = 2
autovalores, autovetores, dados_novo_espaco, erro_quadratico_medio = PCA(dados, m)
print(f"Erro quadratico médio para {m} features: {erro_quadratico_medio}")
